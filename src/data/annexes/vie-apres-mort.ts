import type { EtapeAkhira } from '../types';

export const etapesAkhira: EtapeAkhira[] = [
  {
    titre: "La mort (Al-Mawt)",
    titreArabe: "الموت",
    icon: "Clock",
    contenu: [
      "La mort est la première étape vers l'au-delà. Allah dit : « Toute âme goûtera la mort. Ensuite c'est vers Nous que vous serez ramenés. » (Sourate Al-Ankabut, 29:57). La mort n'est pas une fin, mais un passage vers la vie éternelle.",
      "L'ange de la mort (Malak al-Mawt) vient recueillir l'âme au moment décrété par Allah. Pour le croyant, les anges viennent avec des visages lumineux et un parfum de musc. Pour le mécréant, les anges viennent avec des visages sombres et terrifiants (Ahmad, Abu Dawud).",
      "Le Prophète ﷺ a dit : « Quand le croyant est sur le point de quitter ce monde et d'entrer dans l'autre, des anges au visage blanc comme le soleil descendent du ciel avec un linceul et un parfum du Paradis. » (Ahmad)",
      "Au moment de la mort, il est recommandé de rappeler au mourant la Shahada. Le Prophète ﷺ a dit : « Dictez à vos mourants : La ilaha illa Allah. » (Muslim). La dernière parole d'un croyant devrait être l'attestation de foi."
    ]
  },
  {
    titre: "Les funérailles (Al-Janaza)",
    titreArabe: "الجنازة",
    icon: "Heart",
    contenu: [
      "Le lavage rituel (ghusl al-mayyit) du défunt est une obligation communautaire (fard kifaya). Le corps est lavé un nombre impair de fois avec de l'eau et du sidr (jujubier), puis enveloppé dans un linceul blanc (kafan).",
      "La prière funéraire (Salat al-Janaza) est accomplie en communauté. Elle comporte 4 takbirat (sans ruku' ni sujud) : après le 1er on récite Al-Fatiha, après le 2e la prière sur le Prophète, après le 3e l'invocation pour le défunt, et après le 4e on termine.",
      "L'enterrement doit être fait rapidement. Le Prophète ﷺ a dit : « Hâtez-vous de porter le défunt (à sa tombe). S'il est pieux, c'est un bien vers lequel vous le menez. S'il est autrement, c'est un mal que vous ôtez de vos épaules. » (Bukhari & Muslim)",
      "Le défunt est placé dans la tombe sur son côté droit, face à la Qibla. On dit : « Bismillah wa 'ala millati Rasulillah — Au nom d'Allah et selon la voie du Messager d'Allah. »"
    ]
  },
  {
    titre: "Les épreuves de la tombe (Al-Barzakh)",
    titreArabe: "البرزخ",
    icon: "AlertTriangle",
    contenu: [
      "Le Barzakh est la vie intermédiaire entre la mort et la Résurrection. C'est un monde que nous ne pouvons percevoir, mais dont le Prophète ﷺ nous a informés en détail.",
      "Les deux anges Munkar et Nakir viennent interroger le défunt dans sa tombe. Ils posent trois questions : « Qui est ton Seigneur ? Quelle est ta religion ? Qui est cet homme qui a été envoyé parmi vous ? » Le croyant répond : « Mon Seigneur est Allah. Ma religion est l'Islam. C'est Muhammad, le Messager d'Allah. » (Ahmad, Abu Dawud)",
      "Pour le croyant qui répond correctement, sa tombe est élargie et illuminée, et on lui ouvre une fenêtre vers le Paradis. Il vit dans le bonheur jusqu'au Jour de la Résurrection. Le Prophète ﷺ a dit : « La tombe est soit un jardin parmi les jardins du Paradis, soit une fosse parmi les fosses de l'Enfer. » (Tirmidhi)",
      "La sourate Al-Mulk (67) protège du châtiment de la tombe. Le Prophète ﷺ a dit : « Une sourate du Coran comportant trente versets a intercédé en faveur d'un homme jusqu'à ce qu'il lui soit pardonné : c'est Tabarak alladhi bi-yadihi al-mulk. » (Abu Dawud, Tirmidhi)"
    ]
  },
  {
    titre: "La Résurrection (Al-Ba'th)",
    titreArabe: "البعث",
    icon: "Sunrise",
    contenu: [
      "Israfil soufflera dans la Trompe (as-Sur) et tous les êtres vivants mourront. Puis il soufflera une seconde fois et tous les morts seront ressuscités de leurs tombes. Allah dit : « Et on soufflera dans la Trompe, et voilà que, des tombes, ils se précipiteront vers leur Seigneur. » (Sourate Ya-Sin, 36:51)",
      "Les hommes sortiront de leurs tombes pieds nus, nus et incirconcis, comme ils ont été créés la première fois. Aisha demanda : « Hommes et femmes se regarderont-ils ? » Le Prophète ﷺ répondit : « La situation sera trop grave pour qu'ils se soucient de cela. » (Bukhari & Muslim)",
      "Le soleil sera rapproché des créatures à la distance d'un mile. Les gens transpireront en proportion de leurs péchés : certains jusqu'aux chevilles, d'autres jusqu'aux genoux, d'autres jusqu'à la poitrine, et certains seront noyés dans leur sueur. (Muslim)",
      "Ce jour durera 50 000 ans de la vie d'ici-bas. Allah dit : « Les anges ainsi que l'Esprit (Jibril) montent vers Lui en un jour dont la durée est de cinquante mille ans. » (Sourate Al-Ma'arij, 70:4). Mais pour le croyant, ce jour sera allégé comme le temps entre le Dhuhr et le Asr."
    ]
  },
  {
    titre: "Le Rassemblement et la Reddition des comptes",
    titreArabe: "الحشر والحساب",
    icon: "Scale",
    contenu: [
      "Tous les êtres humains seront rassemblés sur une terre blanche, plate, sans repère. Le Prophète ﷺ a dit que cette terre sera comme un pain de farine pure, sans aucune marque (Bukhari & Muslim).",
      "Chaque personne recevra son livre de comptes (Kitab). Celui qui le recevra dans sa main droite sera jugé avec facilité. Celui qui le recevra dans sa main gauche ou derrière son dos sera châtié. Allah dit : « Quant à celui qui recevra son livre en sa main droite, il dira : Tenez ! Lisez mon livre. » (Sourate Al-Haqqa, 69:19)",
      "Les membres du corps témoigneront des actes de la personne. Allah dit : « Ce jour-là, Nous scellerons leurs bouches, tandis que leurs mains Nous parleront et que leurs pieds témoigneront de ce qu'ils avaient commis. » (Sourate Ya-Sin, 36:65)",
      "Le Prophète ﷺ intercédera pour sa communauté (ash-Shafa'a al-Kubra). Il se prosternera devant Allah et implorera Son pardon pour les croyants pécheurs. C'est le rang louable (Al-Maqam al-Mahmud) promis par Allah dans le Coran (Sourate Al-Isra, 17:79)."
    ]
  },
  {
    titre: "La Balance (Al-Mizan)",
    titreArabe: "الميزان",
    icon: "Scale",
    contenu: [
      "Allah placera les Balances de la justice pour peser les actes des serviteurs. Allah dit : « Nous placerons les balances exactes pour le Jour de la Résurrection. Aucune âme ne sera lésée en rien. Fût-ce le poids d'un grain de moutarde, Nous l'apporterons. Et Nous suffisons pour dresser les comptes. » (Sourate Al-Anbiya, 21:47)",
      "Ce sont de véritables balances avec deux plateaux. Le Prophète ﷺ a dit : « Deux paroles sont légères sur la langue, lourdes dans la balance, aimées du Tout Miséricordieux : SubhanAllahi wa bihamdihi, SubhanAllahi al-Azim — Gloire et louange à Allah, Gloire à Allah le Très Grand. » (Bukhari & Muslim)",
      "La parole « La ilaha illa Allah » est la plus lourde de toutes dans la balance. Le Prophète ﷺ a rapporté qu'un homme viendra avec 99 registres de péchés, chacun aussi long que la vue peut porter. On placera en face une carte portant « La ilaha illa Allah ». La carte l'emportera sur tous les registres. (Tirmidhi)",
      "Celui dont les bonnes actions pèseront plus lourd que les mauvaises entrera au Paradis. Celui dont les mauvaises actions pèseront plus lourd sera en danger, sauf si Allah lui pardonne par Sa miséricorde."
    ]
  },
  {
    titre: "Le Bassin (Al-Hawd)",
    titreArabe: "الحوض",
    icon: "Droplets",
    contenu: [
      "Chaque prophète aura un bassin au Jour de la Résurrection. Le bassin du Prophète Muhammad ﷺ (Al-Hawd ou Al-Kawthar) est le plus grand de tous. Celui qui en boira ne connaîtra plus jamais la soif.",
      "Le Prophète ﷺ a décrit son bassin : « Mon bassin est d'un mois de marche. Son eau est plus blanche que le lait, son odeur plus agréable que le musc, et ses coupes sont aussi nombreuses que les étoiles du ciel. Celui qui en boit ne connaîtra plus jamais la soif. » (Bukhari & Muslim)",
      "Il est alimenté par deux canaux provenant du fleuve Al-Kawthar au Paradis. Allah dit : « Nous t'avons certes accordé l'Abondance (Al-Kawthar). » (Sourate Al-Kawthar, 108:1)",
      "Certains seront repoussés du bassin car ils auront dévié après le Prophète ﷺ. Il dira : « Ce sont mes compagnons ! » Et on lui dira : « Tu ne sais pas ce qu'ils ont innové après toi. » (Bukhari & Muslim)"
    ]
  },
  {
    titre: "Le Pont (As-Sirat)",
    titreArabe: "الصراط",
    icon: "ArrowRight",
    contenu: [
      "Le Sirat est un pont dressé au-dessus de l'Enfer (Jahannam) que tous devront traverser pour atteindre le Paradis. Il est plus fin qu'un cheveu et plus tranchant qu'une épée.",
      "Les gens le traverseront à des vitesses différentes selon leurs actes : certains comme l'éclair, d'autres comme le vent, d'autres comme un cheval au galop, d'autres en marchant, d'autres en rampant, et certains tomberont en Enfer. (Muslim)",
      "Le Prophète ﷺ a dit : « Le premier à traverser sera moi et mon invocation ce jour-là sera : Ô Allah, accorde le salut ! Accorde le salut ! (Allahumma sallim, sallim !) » (Muslim)",
      "Des crochets (kalaalib) seront disposés sur les côtés du pont, semblables aux épines du sa'dan (plante épineuse). Ils attraperont les gens selon leurs actes. Certains seront sauvés intacts, d'autres seront griffés puis sauvés, et d'autres tomberont dans le Feu. (Bukhari & Muslim)"
    ]
  },
  {
    titre: "Le Paradis (Al-Janna)",
    titreArabe: "الجنة",
    icon: "TreePine",
    contenu: [
      "Le Paradis est la demeure éternelle de béatitude préparée par Allah pour les croyants. Allah dit : « Des jardins sous lesquels coulent les ruisseaux, où ils demeureront éternellement. » (Sourate At-Tawba, 9:72). Il contient ce qu'aucun œil n'a vu, ce qu'aucune oreille n'a entendu, et ce qui n'a jamais traversé le cœur d'un être humain. (Bukhari & Muslim)",
      "Le Paradis a huit portes et cent degrés. Entre chaque degré, il y a une distance comparable à celle entre le ciel et la terre. Le Firdaws est le degré le plus élevé, au-dessus duquel se trouve le Trône du Tout Miséricordieux. Le Prophète ﷺ a dit : « Quand vous demandez à Allah, demandez-Lui Al-Firdaws. » (Bukhari)",
      "Les habitants du Paradis seront dans une jeunesse éternelle, sans maladie, sans fatigue, sans tristesse. Ils porteront des vêtements de soie et d'or, mangeront et boiront sans aucune impureté. Ils auront tout ce qu'ils désirent, et au-dessus de tout, la satisfaction d'Allah.",
      "La plus grande récompense au Paradis sera de voir le Visage d'Allah. Le Prophète ﷺ a dit : « Quand les habitants du Paradis seront dans le Paradis, Allah dira : ‹ Voulez-vous quelque chose de plus ? › Ils diront : ‹ N'as-Tu pas blanchi nos visages ? Ne nous as-Tu pas fait entrer au Paradis et sauvés de l'Enfer ? › Alors Il lèvera le voile, et rien de ce qui leur a été donné ne leur sera plus cher que de regarder leur Seigneur. » (Muslim)"
    ]
  },
  {
    titre: "L'Enfer (Jahannam)",
    titreArabe: "جهنم",
    icon: "Flame",
    contenu: [
      "L'Enfer est le châtiment préparé pour les mécréants et les grands pécheurs. Allah dit : « Préservez-vous du Feu dont le combustible est fait d'hommes et de pierres, préparé pour les mécréants. » (Sourate Al-Baqara, 2:24). Il a sept portes, chaque porte étant assignée à une catégorie de gens.",
      "Le feu de l'Enfer est 70 fois plus intense que le feu de ce monde. Le Prophète ﷺ a dit : « Votre feu est un soixante-dixième du feu de l'Enfer. » On dit : « Par Allah, il serait bien suffisant ! » Il dit : « Il lui est supérieur de soixante-neuf parts, chacune ayant la même chaleur. » (Bukhari & Muslim)",
      "Les mécréants y demeureront éternellement. Quant aux musulmans pécheurs qui y entreront, ils en sortiront après avoir purgé leur peine par la miséricorde d'Allah et l'intercession du Prophète ﷺ. Le Prophète ﷺ a dit : « Sortira de l'Enfer quiconque avait dans le cœur le poids d'un grain de moutarde de foi. » (Bukhari & Muslim)",
      "La description de l'Enfer dans le Coran et la Sunna n'est pas pour désespérer, mais pour avertir et encourager le repentir. Allah dit : « Dis : Ô Mes serviteurs qui avez commis des excès à votre propre détriment, ne désespérez pas de la miséricorde d'Allah. Car Allah pardonne tous les péchés. Oui, c'est Lui le Pardonneur, le Très Miséricordieux. » (Sourate Az-Zumar, 39:53)"
    ]
  }
];

export const introAkhira = "La croyance en la vie après la mort (Al-Akhira) est le cinquième pilier de la foi en Islam. C'est l'une des croyances les plus fondamentales qui donne un sens à la vie terrestre et motive le croyant à œuvrer pour le bien. Le Coran et la Sunna décrivent en détail les étapes qui suivent la mort : les épreuves de la tombe, la Résurrection, le Jugement, la Balance, le Pont, et la destination finale — le Paradis ou l'Enfer. Cette page présente ces étapes de manière éducative, basée sur les sources authentiques.";
