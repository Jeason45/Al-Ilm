import type { Metadata } from 'next';
import Link from 'next/link';
import { etapesAkhira, introAkhira } from '@/data/annexes/vie-apres-mort';
import { ScrollReveal } from '@/components/ScrollReveal';
import type { LucideIcon } from 'lucide-react';
import { Clock, Heart, AlertTriangle, Sunrise, Scale, Droplets, ArrowRight, TreePine, Flame } from 'lucide-react';

const iconMap: Record<string, LucideIcon> = {
  Clock, Heart, AlertTriangle, Sunrise, Scale, Droplets, ArrowRight, TreePine, Flame,
};

export const metadata: Metadata = {
  title: 'La vie après la mort',
  description: 'Les étapes de la vie après la mort en Islam : la tombe, la Résurrection, le Jugement, la Balance, le Sirat, le Paradis et l\'Enfer.',
};

export default function VieApresMortPage() {
  return (
    <div style={{ paddingTop: 'clamp(4rem, 8vw, 7rem)', paddingBottom: 'clamp(3rem, 6vw, 6rem)', width: '100%' }}>
      <div style={{ width: '100%', maxWidth: '1100px', marginLeft: 'auto', marginRight: 'auto', paddingLeft: 'clamp(14px, 4vw, 24px)', paddingRight: 'clamp(14px, 4vw, 24px)' }}>
        <Link href="/annexes" className="back-link">
          <span className="back-link-icon">←</span> Annexes
        </Link>
        <ScrollReveal>
          <div style={{ textAlign: 'center', marginBottom: '3rem' }}>
            <p className="font-amiri text-gold" style={{ fontSize: '1.75rem', marginBottom: '1rem', opacity: 0.5 }}>
              الآخرة
            </p>
            <h1 className="font-outfit font-bold" style={{ fontSize: 'clamp(2.25rem, 5vw, 3.5rem)', letterSpacing: '-0.03em', marginBottom: '1rem' }}>
              La vie après la mort.
            </h1>
            <p className="text-muted" style={{ fontSize: '1.0625rem', maxWidth: '36rem', margin: '0 auto', lineHeight: 1.7 }}>
              {introAkhira}
            </p>
          </div>
        </ScrollReveal>

        <div style={{ maxWidth: '48rem', margin: '0 auto' }}>
          {etapesAkhira.map((etape, index) => {
            const Icon = iconMap[etape.icon] || Clock;
            return (
              <ScrollReveal key={etape.titre} delay={index * 50}>
                <div style={{ position: 'relative', paddingLeft: '2.5rem', paddingBottom: index < etapesAkhira.length - 1 ? '1.5rem' : 0 }}>
                  {/* Timeline line */}
                  {index < etapesAkhira.length - 1 && (
                    <div style={{
                      position: 'absolute', left: '7px', top: '20px', bottom: 0, width: '1px',
                      background: 'var(--color-border)',
                    }} />
                  )}
                  {/* Timeline dot */}
                  <div style={{
                    position: 'absolute', left: 0, top: '8px', width: '15px', height: '15px',
                    borderRadius: '50%', background: 'var(--color-gold)',
                  }} />

                  <div className="surah-card" style={{ padding: 'clamp(1.25rem, 2.5vw, 1.75rem)' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '0.5rem' }}>
                      <Icon style={{ width: '18px', height: '18px', color: 'var(--color-gold)', flexShrink: 0 }} />
                      <h3 className="font-outfit font-semibold" style={{ fontSize: '1.0625rem' }}>{etape.titre}</h3>
                      <span className="font-amiri text-gold" style={{ fontSize: '1rem', opacity: 0.6 }}>{etape.titreArabe}</span>
                    </div>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.625rem', marginTop: '0.5rem' }}>
                      {etape.contenu.map((paragraphe, j) => (
                        <p key={j} style={{ fontSize: '0.9375rem', lineHeight: 1.7 }}>{paragraphe}</p>
                      ))}
                    </div>
                  </div>
                </div>
              </ScrollReveal>
            );
          })}
        </div>
      </div>
    </div>
  );
}
