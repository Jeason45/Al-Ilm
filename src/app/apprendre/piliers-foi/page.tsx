...
import { piliersFoi, introPiliersFoi } from '@/data/annexes/piliers-foi';
import { ScrollReveal } from '@/components/ScrollReveal';
import type { LucideIcon } from 'lucide-react';
import { Sparkles, Wind, BookOpen, Users, Hourglass, Infinity } from 'lucide-react';

const iconMap: Record<string, LucideIcon> = {
  Sparkles, Wind, BookOpen, Users, Hourglass, Infinity,
};

export const metadata: Metadata = {
  title: 'Les 6 piliers de la foi',
  description: 'Les six piliers de la foi en Islam (Arkan al-Iman) : foi en Allah, aux Anges, aux Livres, aux Prophètes, au Jour Dernier et au Destin.',
};

export default function PiliersFoiPage() {
  return (
    <div style={{ paddingTop: 'clamp(4rem, 8vw, 7rem)', paddingBottom: 'clamp(3rem, 6vw, 6rem)', width: '100%' }}>
      <div style={{ width: '100%', maxWidth: '1100px', marginLeft: 'auto', marginRight: 'auto', paddingLeft: 'clamp(14px, 4vw, 24px)', paddingRight: 'clamp(14px, 4vw, 24px)' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          {piliersFoi.map((pilier, i) => {
            const Icon = iconMap[pilier.icon] || Sparkles;
            return (
