...
function ChapterHeader({ item }: { item: Extract<TimelineItem, { type: 'chapter' }> }) {
  return (
    <ScrollReveal>
      <div style={{ textAlign: 'center', padding: '3rem 1rem', marginTop: '3rem', marginBottom: '1rem' }}>
        <span style={{
          fontSize: '0.75rem', fontWeight: 600, textTransform: 'uppercase',
          letterSpacing: '0.15em', color: 'var(--color-gold)', display: 'block', marginBottom: '0.5rem',
        }}>
          Chapitre {item.numero}
        </span>
        <h2 className="font-outfit font-bold" style={{ fontSize: 'clamp(1.5rem, 3.5vw, 2rem)', letterSpacing: '-0.02em' }}>
          {item.titre}
        </h2>
      </div>
    </ScrollReveal>
  ...
  return (
    <ScrollReveal>
      <div style={{ textAlign: 'center', padding: '1.5rem 1rem', marginBottom: '1.5rem' }}>
        ...
        <h3 className="font-outfit font-semibold" style={{ fontSize: '1.125rem', marginBottom: '0.375rem' }}>
          {item.titre}
        </h3>
    ...
    <ScrollReveal>
      <div style={{ position: 'relative', paddingLeft: 'clamp(2rem, 5vw, 2.5rem)', paddingBottom: isLast ? 0 : '2rem' }}>
        ...
        <div className="surah-card" style={{ padding: 'clamp(1.25rem, 2.5vw, 1.75rem)' }}>
          <span style={{
            fontSize: '0.75rem', color: 'var(--color-gold)', fontWeight: 500,
            textTransform: 'uppercase', letterSpacing: '0.05em', display: 'block', marginBottom: '0.5rem',
          }}>
            {item.date}
          </span>

          <h3 className="font-outfit font-semibold" style={{ fontSize: '1.0625rem', marginBottom: '1rem' }}>
            {item.id}. {item.titre}
          </h3>
          ...
          <div style={{
            ...
            <span style={{
              fontSize: '0.75rem', fontWeight: 600, textTransform: 'uppercase',
              letterSpacing: '0.1em', color: 'var(--color-gold)', display: 'block', marginBottom: '0.375rem',
            }}>
              Leçon
            </span>
          ...
          {item.sourates && (
            <div style={{ marginTop: '0.75rem' }}>
              <span style={{ fontSize: '0.75rem', color: 'var(--color-muted)', display: 'inline-flex', alignItems: 'center', gap: '0.375rem' }}>
    ...
    <div style={{ maxWidth: '700px', margin: '0 auto' }}>
      <ScrollReveal>
        <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
          <p className="text-muted" style={{ fontSize: '1.0625rem', maxWidth: '32rem', margin: '0 auto', marginBottom: '0.75rem' }}>
      ...
      <div style={{ maxWidth: '40rem', margin: '0 auto' }}>
        {histoireTimeline.map((item, index) => {
          if (item.type === 'chapter') return <ChapterHeader key={`ch-${index}`} item={item} />;
          if (item.type === 'section') return <SectionHeader key={`sec-${index}`} item={item} />;
          return <EventCard key={`ev-${item.id}`} item={item} isLast={item.id === lastEventId} />;
        })}
      </div>

      <ScrollReveal delay={200}>
        <div style={{ marginTop: '4rem', textAlign: 'center', padding: '2rem 1rem', borderTop: '1px solid var(--color-border)' }}>
          <p className="font-amiri text-gold" style={{ fontSize: '1.5rem', marginBottom: '1.5rem', opacity: 0.5 }}>
            الخاتمة
          </p>
          {histoireMeta.epilogue.split('\n\n').map((paragraph, i) => (
            <p key={i} style={{
              fontSize: '1.0625rem', lineHeight: 1.8, color: 'var(--color-text)',
              maxWidth: '32rem', margin: '0 auto', marginBottom: '1rem', opacity: 0.85,
            }}>
              {paragraph.split('\n').map((line, j) => (
